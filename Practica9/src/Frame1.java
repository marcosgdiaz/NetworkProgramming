...
class Frame1 implements Cons1 {
	...
	boolean firstDATAframe () {
		return(((code(rec)==DATA)||code(rec)==ERROR)&&(remoteTID==ServerPort));
	...
	void printframe (DatagramPacket dp) {
		...
		if (code(dp) == RRQ) {
			System.out.println("RRQ" + " file=" + file(dp) + 
					" mode=" + mode(dp) + " remotePort:" + dp.getPort()+
					" len:" +dp.getLength() + " Addr:" + dp.getAddress());
		};
		if (code(dp) == WRQ) {
		...
		if (code(dp) == ACK) {
			System.out.println("ACK" + " seqnum=" + seqnum(dp)+
				 " remotePort:" + dp.getPort()+
					" len:" +dp.getLength() + " Addr:" + dp.getAddress());};
		if (code(dp) == ERROR) {
			System.out.println("ERROR " + errormsg(rec));};
		} //printframe
